...
class AttING(nn.Module):
    ...
    def forward(self, x):
        # ipdb.set_trace()
        # x: torch.Size([4, 6, 256, 256])
        x1 = self.conv1(x)  # torch.Size([4, 16, 256, 256])
        out_instance = self.instance(x1)  # torch.Size([4, 16, 256, 256])
        out_identity = x1  # torch.Size([4, 16, 256, 256])
        out1 = self.conv2_1(out_instance)  # torch.Size([4, 16, 256, 256])
        out2 = self.conv2_2(out_identity)  # torch.Size([4, 16, 256, 256])
        out = torch.cat((out1, out2), 1)  # torch.Size([4, 32, 256, 256])
        xp1 = self.interative(out) * out2 + out1  # torch.Size([4, 16, 256, 256])
        xp2 = (1 - self.interative(out)) * out1 + out2  # torch.Size([4, 16, 256, 256])
        xp = torch.cat((xp1, xp2), 1)  # torch.Size([4, 32, 256, 256])
        xp = self.process(self.contrast(xp) + self.avgpool(xp)) * xp
        xp = self.conv1x1(xp)  # torch.Size([4, 16, 256, 256])
        xout = xp
...
class DRBN_BUAtt(nn.Module):
    ...
    def part_forward(self, x):
        ...
        f_first, instancef = self.SFENet1(input_x)  # torch.Size([4, 16, 256, 256])
        f_s1 = self.Relu(self.SFENet2(self.Relu(f_first)))  # torch.Size([4, 16, 256, 256])
        f_s2 = self.Down1(self.RDBs[0](f_s1))  # torch.Size([4, 16, 128, 128])
        f_s4 = self.Down2(self.RDBs[1](f_s2))  # torch.Size([4, 32, 64, 64])

        if flag == 0:
            f_s4 = f_s4 + self.RDBs[3](self.RDBs[2](f_s4))  # torch.Size([4, 32, 64, 64])
            f_s2 = f_s2 + self.RDBs[4](self.Up2(f_s4))  # torch.Size([4, 16, 128, 128])
            f_s1 = f_s1 + self.RDBs[5](self.Up1(f_s2)) + f_first  # torch.Size([4, 16, 256, 256])
        else:
        ...
        res4 = self.UPNet4(f_s4)  # torch.Size([4, 3, 64, 64])
        res2 = self.UPNet2(f_s2) + self.Img_up(res4)  # torch.Size([4, 3, 128, 128])
        res1 = self.UPNet(f_s1) + self.Img_up(res2)  # torch.Size([4, 3, 256, 256])
