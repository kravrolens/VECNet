...
class NAFNet(nn.Module):
    ...
    def __init__(self, nframes=1, img_channel=3, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[]):
        super().__init__()
    ...
    def forward(self, inp):
        ...
        # x = self.intro(inp.view(B, N*C, H, W))
        B, N, C, H, W = inp.shape
...
class framesBlock(nn.Module):
    def __init__(self, channels=[32, 16, 16, 3], block=ConvBlock):
        super().__init__()
        ...
        for i in range(len(channels)):
            ...
            self.blocks.append(block(in_channel, out_channel))
